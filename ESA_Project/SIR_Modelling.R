...
# but for our ESA Project we will focus on act.rate — Average number of transmissible acts, like shaking hands.
...
simulate_model<-function(degree=0.5){
  ...
  #we first plot the chosen degree of interaction decided by players
  ...
  lines(mod.df$time, mod.df$i.num,
        col=rgb(170/255,1/255,20/255,min(1-rt+.1, 1)),
        lwd=1+(1-rt))   
